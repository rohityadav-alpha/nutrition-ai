...
export async function getAnalyticsData() {
  try {
    ...
    const mealDistribution = Object.entries(mealTypeCount).map(([name, value]) => ({
    ...
    const avgCalories = totalMeals > 0 
      ? Math.round(meals.reduce((sum, m) => sum + m.totalCalories, 0) / totalMeals)
      : 0;
    ...
    // Macro percentage
    const totalMacros = totalProtein * 4 + totalCarbs * 4 + totalFats * 9; // calories from macros
    const macroDistribution = totalMacros > 0 ? [
      { name: "Protein", value: Math.round((totalProtein * 4 / totalMacros) * 100), color: "#ef4444" },
      { name: "Carbs", value: Math.round((totalCarbs * 4 / totalMacros) * 100), color: "#f97316" },
      { name: "Fats", value: Math.round((totalFats * 9 / totalMacros) * 100), color: "#eab308" },
    ] : [];
