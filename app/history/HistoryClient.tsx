...
export default function HistoryClient() {
  ...
  const loadData = async (opts?: {
    ...
    if (mealsRes.success) {
      ...
      if (!hasActiveFilters(currentFilters)) {
        // ❌ koi filter nahi → backend ke last‑7‑days stats use karo
        if (statsRes.success && statsRes.stats) {
          setStats(statsRes.stats as WeeklyStats);
          setBaseStats(statsRes.stats as WeeklyStats);
        }
      } else {
        // ✅ filters active → list ke basis pe stats bana do
    ...
    // first load pe baseStats set karo (backup ke liye)
  ...
  const statsSubtitle = hasActiveFilters()
    ? "For selected range"
    : "Last 7 days";
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-blue-50 to-purple-50 py-8 px-4 text-gray-700">
      <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
            ...
            <div className="flex space-x-2">
              <button
                ...
                className="flex-1 bg-gradient-to-r from-blue-600 to-purple-600 text-white font-semibold py-2 px-4 rounded-lg hover:shadow-md transition-all"
              >
                Apply
              </button>
              <button
                onClick={handleClearFilters}
                className="flex-1 bg-gray-100 text-gray-700 font-semibold py-2 px-4 rounded-lg hover:bg-gray-200 transition-all"
              >
                Clear
              </button>
            </div>
        ...
        {/* Stats cards (overall or filtered) */}
        {stats && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 mb-8">
            <div className="bg-white rounded-xl shadow-lg p-6 border border-blue-100">
              ...
              <p className="text-xs text-gray-500 mt-1">{statsSubtitle}</p>
            </div>
            ...
            <div className="bg-white rounded-xl shadow-lg p-6 border border-purple-100">
              ...
              <p className="text-xs text-gray-500 mt-1">
                {hasActiveFilters() ? "Per meal (selected range)" : "Per meal"}
              </p>
            </div>
